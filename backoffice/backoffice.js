...
function emptyItem(){return{image:"",photos:[],title:"",category:"creations",icon:"🎨",description:"",long_description:"",details:"",dimensions:"",duration:"",featured:false,draft:false,date:""}}
function normalizeItem(item){
  const d=item.description_section||{}, i=item.images_section||{}, info=item.infos_section||{};
  return {image:item.image||i.image||"",photos:item.photos||i.photos||[],title:item.title||d.title||"",category:item.category||d.category||"creations",icon:item.icon||d.icon||"",description:item.description||d.description||"",long_description:item.long_description||d.long_description||"",details:item.details||info.details||"",dimensions:item.dimensions||info.dimensions||"",duration:item.duration||info.duration||"",featured:item.featured??info.featured??false,draft:item.draft??false,date:item.date||info.date||""}
}
...
function saveForm(){if(selected<0)return;const it=data.body[selected];["image","title","icon","description","long_description","details","dimensions","duration"].forEach(id=>it[id]=$(id).value.trim());it.category=$("category").value;it.featured=$("featured").checked;it.draft=$("draft").checked;it.date=$("date").value||"";it.photos=[...document.querySelectorAll(".photo-row")].map(row=>({photo:row.querySelector(".photo-path").value.trim(),caption:row.querySelector(".photo-caption").value.trim()})).filter(p=>p.photo)}
function loadForm(){if(selected<0){$("formTitle").textContent="Aucune réalisation";return}const it=data.body[selected];$("formTitle").textContent=it.title||"Nouvelle réalisation";["image","title","icon","description","long_description","details","dimensions","duration"].forEach(id=>$(id).value=it[id]||"");$("category").value=it.category||"creations";$("featured").checked=!!it.featured;$("draft").checked=!!it.draft;$("date").value=(it.date||"").slice(0,10);$("mainPreview").src=it.image||"/assets/logo.png";$("photos").innerHTML="";(it.photos||[]).forEach(addPhotoRow)}
function addPhotoRow(photo={photo:"",caption:""}){const row=document.createElement("div");row.className="photo-row";row.innerHTML=`<img src="${esc(photo.photo||'/assets/logo.png')}" ondblclick="openImageModal(this.src)"><input class="photo-path" placeholder="/assets/uploads/photo.jpg" value="${esc(photo.photo||"")}"><input class="photo-caption" placeholder="Légende" value="${esc(photo.caption||"")}"><button type="button" title="Image principale">⭐</button><button type="button" title="Monter">↑</button><button type="button" title="Descendre">↓</button><button type="button" title="Supprimer">×</button>`;const buttons=row.querySelectorAll("button");buttons[0].onclick=()=>{saveForm();const path=row.querySelector(".photo-path").value;const cur=$("image").value;$("image").value=path;row.querySelector(".photo-path").value=cur;$("mainPreview").src=path||"/assets/logo.png";saveForm();render()};buttons[1].onclick=()=>{if(row.previousElementSibling)$("photos").insertBefore(row,row.previousElementSibling)};buttons[2].onclick=()=>{if(row.nextElementSibling)$("photos").insertBefore(row.nextElementSibling,row)};buttons[3].onclick=()=>row.remove();$("photos").appendChild(row)}
function renderList(){const q=($("searchList")?.value||"").toLowerCase();$("list").innerHTML=data.body.map((it,i)=>({it,i})).filter(x=>!q||(x.it.title+" "+x.it.description).toLowerCase().includes(q)).map(({it,i})=>`<div class="item ${i===selected?'active':''} ${it.draft?'draft':''}" data-i="${i}"><img src="${it.image||'/assets/logo.png'}"><div><strong>${esc(it.title||'Sans titre')}</strong><small>${esc(it.category||'')}${it.featured?' • À la une':''}${it.draft?' • Brouillon':''}</small></div></div>`).join("");document.querySelectorAll(".item").forEach(el=>el.onclick=()=>{saveForm();selected=Number(el.dataset.i);render()})}

function renderPhotos(){
  const imgs=[];
  data.body.forEach(r=>{
    if(r.image)imgs.push(r.image);
    (r.photos||[]).forEach(p=>imgs.push(p.photo));
  });
  const unique=[...new Set(imgs)].filter(Boolean);
  const q=($("librarySearch")?.value||"").toLowerCase();
  const filtered=unique.filter(src=>src.toLowerCase().includes(q));
  const grid=$("photoGrid");
  if(!grid)return;
  grid.innerHTML=filtered.map(src=>`<div class="libItem ${src===selectedLibraryImage?'selected':''}" data-src="${esc(src)}"><img src="${esc(src)}" ondblclick="openImageModal(this.src)" title="${esc(src)}"><small>${esc(src.split('/').pop())}</small></div>`).join("");
  document.querySelectorAll(".libItem").forEach(el=>el.onclick=()=>{
    selectedLibraryImage=el.dataset.src;
    renderPhotos();
  });
}
function stats(){$("countTotal").textContent=data.body.length;$("countFeatured").textContent=data.body.filter(x=>x.featured&&!x.draft).length;$("countPhotos").textContent=data.body.reduce((n,x)=>n+1+(x.photos||[]).length,0);$("countDrafts").textContent=data.body.filter(x=>x.draft).length}
...
async function publish(){try{saveForm();validateBeforePublish();$("publishBtn").disabled=true;$("publishBtn").textContent="Publication...";const path="content/realisations.json";const current=await gh(path);const content=toBase64Utf8(JSON.stringify(cleanData(),null,2));await gh(path,{method:"PUT",body:JSON.stringify({message:"Publication depuis le back-office V10",content,sha:current.sha,branch:settings().branch})});alert("Publication envoyée sur GitHub. Cloudflare va redéployer le site automatiquement.")}catch(e){alert("Erreur publication : "+e.message)}finally{$("publishBtn").disabled=false;$("publishBtn").textContent="🚀 Publier"}}
...
function setupDrop(id,inputId,callback){const dz=$(id), input=$(inputId);["dragenter","dragover"].forEach(ev=>dz.addEventListener(ev,e=>{e.preventDefault();dz.classList.add("drag")}));["dragleave","drop"].forEach(ev=>dz.addEventListener(ev,e=>{e.preventDefault();dz.classList.remove("drag")}));dz.addEventListener("drop",e=>{input.files=e.dataTransfer.files;callback()})}
...
function addSelectedToGallery(){
  if(!selectedLibraryImage)return alert("Sélectionne d'abord une image dans la bibliothèque.");
  addPhotoRow({photo:selectedLibraryImage,caption:""});
  saveForm();
  render();
}
function useSelectedAsMain(){
  if(!selectedLibraryImage)return alert("Sélectionne d'abord une image dans la bibliothèque.");
  $("image").value=selectedLibraryImage;
  $("mainPreview").src=selectedLibraryImage;
  saveForm();
  render();
}
function syncLibrarySearch(){
  if($("librarySearch"))$("librarySearch").addEventListener("input",renderPhotos);
}
...
$("settingsBtn").onclick=()=>document.querySelector('[data-view="settings"]').click();$("saveSettingsBtn").onclick=saveSettings;$("testGithubBtn").onclick=testGithub;$("clearTokenBtn").onclick=()=>{localStorage.removeItem("gh_token");loadSettings()};$("publishBtn").onclick=publish;$("newBtn").onclick=newItem;$("duplicateBtn").onclick=duplicate;$("deleteBtn").onclick=del;$("addPhotoBtn").onclick=()=>addPhotoRow();$("uploadMainBtn").onclick=uploadMain;$("uploadExtraBtn").onclick=uploadExtra;$("suggestBtn").onclick=suggestText;$("form").onsubmit=e=>{e.preventDefault();saveForm();render();alert("Enregistré localement. Clique sur Publier pour envoyer sur GitHub.")};$("image").addEventListener("input",()=>{$("mainPreview").src=$("image").value||"/assets/logo.png"});$("searchList").addEventListener("input",renderList);
if($("refreshLibraryBtn"))$("refreshLibraryBtn").onclick=renderPhotos;if($("addSelectedToGalleryBtn"))$("addSelectedToGalleryBtn").onclick=addSelectedToGallery;if($("useAsMainBtn"))$("useAsMainBtn").onclick=useSelectedAsMain;syncLibrarySearch();loadSettings();loadSiteData();setupDrop("mainDrop","mainUpload",uploadMain);setupDrop("extraDrop","extraUpload",uploadExtra);
